...
| `insta --agent secrets` [`--branch <name>`] [`--service <compute/name>`] [`-o <file>`] [`--print`] [`--json`] | secret seam → write the branch's secrets to `./.env` (gated: `secrets.read`). Carries user-defined project/branch secrets **plus the branch's canonical provider credentials** — one `DATABASE_URL` / `REDIS_URL` / `AWS_*` set from the **primary** service of each type (see **Provider credentials** below). **`--service <compute/name>` (CLI ≥ 0.0.64)** answers with **one compute service's own** env instead of the branch-wide merge: that service's user secrets, the unbound ones, its explicit bindings, and the same canonical credentials. Needed when several services define the same name — see **Same-name variables** below |
...
| `insta --agent secrets unset <NAME> [--branch <b>] [--service <compute/name>] [--json]` | Remove a user secret. **`--service` (CLI ≥ 0.0.64)** removes **only that service's copy**, leaving a sibling's same-name value alone; without it the original name-keyed delete removes every matching copy at the scope |
...
**Exit 2 is not only approvals (CLI ≥ 0.0.64).** `insta --agent run` reuses it to refuse on a
...
**Same-name variables across services (CLI ≥ 0.0.64).** Per-service env is allowed, so two compute
...
```
insta --agent build ./app --port <n>               # first: verify — `deployable` only with a Dockerfile IN ./app
insta --agent deploy ./app --port <n>              # build ./app's Dockerfile remotely on Fly, push, deploy
...
**Custom domain (bring your own):** `insta --agent compute set-domain app.example.com` attaches your own
...
(Let's Encrypt) + routing are handled for you; `insta --agent compute check-domain <host>` shows status once
...
> scale`/`upgrade`, and **source-directory deploy** (`insta --agent deploy <dir>` → Fly remote builder of the dir's own Dockerfile, no

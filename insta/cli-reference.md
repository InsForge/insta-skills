...
- **GitHub URLs** (⚠️ **preview**, see the note below). `insta template deploy https://github.com/<owner>/<repo>/tree/<ref>/<dir>` reads
  ...
  > `https:/` with a single slash is the tell, not a missing file. `insta upgrade` fixes it.

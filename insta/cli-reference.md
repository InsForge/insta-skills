...
| `insta compute restart [service]` [`--branch <b>`] [`--json`] | **re-run the image the service already runs**, against a freshly resolved env bundle — no new image, no new spec. The two reasons to use it: a secret/binding changed and the running app hasn't picked it up (env is baked into the machine at deploy time), or the machine is up but **wedged** (`start` no-ops on a machine that is already `started`). The service must be **running**: a deliberately stopped/suspended one 400s and points at `insta compute start`. A never-deployed service 400s like `exec` does. **Health-gated coming back up** — if the app doesn't answer on its port the machines are rolled back to the config they were serving and the failure is reported (that verdict means the app is broken, not the platform). All plans; ungated; bills as ordinary uptime, and refused while the org is billing-suspended. Does **not** restore `--websocket` concurrency (not recorded on the service) — redeploy for those |

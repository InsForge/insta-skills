...
| `insta --agent domain buy <name>` [`--years n`] [`--branch --group`] [`--contact-file c.json`] [`--json`] | order it. Answers a **Stripe Checkout URL a human must open** — nothing is registered until they pay, and registrations are **non-refundable**, so relay the URL and stop. Gated: `domain.purchase` (approval tier → 202 + exit 2, see [Approval relay](SKILL.md#approval-relay-critical--gated-actions)). Needs a registrant contact: the org default (`domain contact set`) or `--contact-file`. Some registries take fixed terms (`.ai` is 2-year only) — a term they refuse is a 400 **before** any payment |
...
| `insta --agent domain contact` · `insta --agent domain contact set` [flags \| `--contact-file c.json`] [`--org`] | the org's default **registrant contact** — the legal owner of every domain bought with it. `--company-name` makes that organization the registrant instead of the person (ICANN). `set` is admin-only and **human-only**: agents pass a contact the human gave them |

...
| `insta db query <service> <query…>` [`--database <db>`] [`--branch <b>`] [`--json`] | run one query against a **managed** database through the console exec API — mysql (a SQL statement), mongodb (a mongosh eval expression; `--database` picks which db, default `admin` — **mongodb-only**, errors on other engines), or redis (a pre-tokenized command, e.g. `GET mykey`). `<service>` is the service name; the engine is resolved from it. **Not** for postgres/compute/storage (errors — postgres has its own SQL editor / `DATABASE_URL`). An empty query errors before any request. Gated: `db.query`. Output: an aligned table (mysql) / raw reply (redis) / pretty EJSON (mongodb), or the raw JSON body with `--json` |
